...
router.post("/investments2", async (req, res) => {
  ...
  try {
    ...
    if (bFound && (await bcrypt.compare(password, user.password))) {
      ...
      // user
      res.redirect("http://localhost:4000/principal/?username=" + user.firstName + "&tickets=" + ticketCount + "&customerid=" + user._id).status(200).json(user);
    }
