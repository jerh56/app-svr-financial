const { Router } = require('express');
const userModel = require("../models");
const { GoogleSpreadsheet } = require('google-spreadsheet');
const jwt = require("jsonwebtoken");
const bcrypt = require("bcryptjs");
const router = Router();
const auth = require("../middleware/auth");

//Raiz
router.get('/', (req, res) => {
  res.json(
    {
      "message": "Holding"
    }
  );
})

router.post("/login", async (req, res) => {

  // Our login logic starts here
  try {
    // Get user input
    const { email, password } = req.body;

    // Validate user input
    if (!(email && password)) {
      //res.status(400).send("All input is required");
      res.redirect("https://mholding.herokuapp.com/error/?message=Se deben capturar todos los campos");
    }
    // Validate if user exist in our database
    //const user = await User.findOne({ email });

    // Initialize the sheet - doc ID is the long id in the sheets URL
    const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');

    // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
    await doc.useServiceAccountAuth({
      // env var values are copied from service account credentials generated by google
      // see "Authentication" section in docs for more info
      client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
      private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
    });

    await doc.loadInfo(); // loads document properties and worksheets
    //console.log(doc.title);
    // await doc.updateProperties({ title: 'renamed doc' });

    const sheet = doc.sheetsByIndex[0]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
    // read rows
    const rows = await sheet.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
    let bFound = false;
    let user = { _id: "", token: "", firstName:"", password:"" };
    let rowCount = rows.length;
    for (let i = 0; i < rowCount; i++) {
      if (rows[i].email == email) {
          bFound = true;
          user._id = rows[i].customerID;
          user.password = rows[i].key;
          user.firstName = rows[i].firstName
          break;
      }
    }

    if (bFound && (await bcrypt.compare(password, user.password))) {
      // Create token
      const token = jwt.sign(
        { user_id: user._id, email },
        process.env.TOKEN_KEY,
        {
          expiresIn: "2h",
        }
      );

      // save user token
      user.token = token;

      const sheet2 = doc.sheetsByIndex[3]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
      // read rows
      const rows2 = await sheet2.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
      bFound = false;
      rowCount = rows2.length;
      let ticketCount = 0;
      for (let i = 0; i < rowCount; i++) {
        console.log(rows2[i].investmentID);
        if (rows2[i].customerID == user._id) {
            ticketCount++;
        }
      }

      // user
      // Para Prod usarhttps://mholding.herokuapp.com 
      res.redirect("https://mholding.herokuapp.com/principal/?username="+ user.firstName +"&tickets=" + ticketCount + "&customerid=" + user._id).status(200).json(user);
    }
    //res.status(400).send("Invalid Credentials");
    res.redirect("https://mholding.herokuapp.com/error/?message=El usuario o contraseña no son correctos");
  } catch (err) {
    console.log(err);
  }
  // Our register logic ends here
});


router.post("/register", async (req, res) => {

  // Our login logic starts here
  try {
    // Get user input
    const { email, password, firstName, lastName } = req.body;
    console.log (req.body);

    // Validate user input
    if (!(email && password && firstName && lastName)) {
      //res.status(400).send("All input is required");
      res.redirect("https://mholding.herokuapp.com/error/?message=Se deben capturar todos los campos");
      //process.exit(1);
    }
    // Validate if user exist in our database
    //const user = await User.findOne({ email });

    // Initialize the sheet - doc ID is the long id in the sheets URL
    const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');

    // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
    await doc.useServiceAccountAuth({
      // env var values are copied from service account credentials generated by google
      // see "Authentication" section in docs for more info
      client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
      private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
    });

    await doc.loadInfo(); // loads document properties and worksheets
    //console.log(doc.title);
    // await doc.updateProperties({ title: 'renamed doc' });

    const sheet = doc.sheetsByIndex[0]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
    // read rows
    const rows = await sheet.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
    let bFound = false;
    let rowCount = rows.length;
    for (let i = 0; i < rowCount; i++) {
      if (rows[i].email == email) {
          bFound = true;
          /*user._id = rows[i].customerID;
          user.password = rows[i].key;
          user.firstName = rows[i].firstName*/
          break;
      }
    }

    if (bFound) {
      //res.status(409).send("El usuario con el email: " + email + " ya está registrado");
      res.status(409);
      res.redirect("https://mholding.herokuapp.com/error/?message=El usuario con el email: " + email + " ya está registrado");
      //process.exit(1);
    }

    console.log ("Rows:" , rowCount);

    //rows[rowCount].email = email;
    let user = { _id: rowCount + 1 , firstName: firstName, lastName: lastName, email: email, key: "" };
    console.log(user);
    user.key = await bcrypt.hash(password, 10);
    console.log(user);
    // append rows
    const newRow = await sheet.addRow({ customerID: user._id, firstName: user.firstName , lastName: user.lastName,  email: user.email, key: user.key });
    // Para Prod usarhttps://mholding.herokuapp.com 
    res.redirect("https://mholding.herokuapp.com/principal/?username=" + user.firstName + "&tickets=" + 0 + "&customerid=" + user._id);  
    //process.exit(0);
    //await rows[rowCount].save(); // save updates
    //res.status(200).send("Successful registration");
/*
    console.log("pass:", await bcrypt.hash(password, 10));
    if (bFound && (await bcrypt.compare(password, user.password))) {
      // Create token
      const token = jwt.sign(
        { user_id: user._id, email },
        process.env.TOKEN_KEY,
        {
          expiresIn: "2h",
        }
      );

      // save user token
      user.token = token;

      const sheet2 = doc.sheetsByIndex[3]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
      // read rows
      const rows2 = await sheet2.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
      bFound = false;
      rowCount = rows2.length;
      let ticketCount = 0;
      for (let i = 0; i < rowCount; i++) {
        console.log(rows2[i].investmentID);
        if (rows2[i].customerID == user._id) {
            ticketCount++;
        }
      }

      // user
      // Para Prod usar https://mholding.herokuapp.com 
      //res.redirect("https://mholding.herokuapp.com/principal/?username="+ user.firstName +"&tickets=" + ticketCount + "&customerid=" + user._id).status(200).json(user);
   
    }
    res.status(400).send("Invalid Credentials"); */
  } catch (err) {
    console.log(err);
    //res.status(500).send(err);
  }
  // Our register logic ends here
});

router.get('/proyectos', async (req, res) => {

  // Initialize the sheet - doc ID is the long id in the sheets URL
  const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');
  // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
  await doc.useServiceAccountAuth({
    // env var values are copied from service account credentials generated by google
    // see "Authentication" section in docs for more info
    client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
    private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
  });

  await doc.loadInfo(); // loads document properties and worksheets
  console.log(doc.title);
  // await doc.updateProperties({ title: 'renamed doc' });

  const sheet = doc.sheetsByIndex[1]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
  console.log(sheet.title);
  console.log(sheet.rowCount);
  // read rows
  const rows = await sheet.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
  //let bFound = false;
  let projects = [];
  let project = { investment_projectID: "", name: "", ticketQuantity: "", projectAmount: "", yearlyRate: "" };
  let rowCount = rows.length;
  let projectCount = 0;
  for (let i = 0; i < rowCount; i++) {
    console.log(rows[i].investment_projectID);
    projectCount++;
    project.investment_projectID = rows[i].investment_projectID;
    project.name = rows[i].name;
    project.ticketQuantity = rows[i].ticketQuantity;
    project.yearlyRate = rows[i].yearlyRate;
    project.projectAmount = rows[i].projectAmount;
    console.log(project);
    projects.push(project);
    project = { investment_projectID: "", name: "", ticketQuantity: "", projectAmount: "", yearlyRate: "" };
  }
  res.status(200).json(projects);
});


router.get('/investments', async (req, res) => {
  console.log("Body:", req.body);
  console.log("Query:", req.query);

  let customerID = req.query.customerid;

  // Initialize the sheet - doc ID is the long id in the sheets URL
  const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');
  // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
  await doc.useServiceAccountAuth({
    // env var values are copied from service account credentials generated by google
    // see "Authentication" section in docs for more info
    client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
    private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
  });

  await doc.loadInfo(); // loads document properties and worksheets
  console.log(doc.title);
  // await doc.updateProperties({ title: 'renamed doc' });

  const sheet = doc.sheetsByIndex[3]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
  console.log(sheet.title);
  console.log(sheet.rowCount);
  // read rows
  const rows = await sheet.getRows({ limit: 100, offset: 0 }); // can pass in { limit, offset }
  //let bFound = false;
  let investments = [];
  let investment = { investmentID: "", investment_projectID: "", investmentAmount: "", customerID: "", ticketID: "" };
  let rowCount = rows.length;
  for (let i = 0; i < rowCount; i++) {
    console.log(rows[i].investment_projectID);
    if (rows[i].customerID == customerID) {
      investment.investmentID = rows[i].investmentID;
      investment.investment_projectID = rows[i].investment_projectID;
      investment.investmentAmount = rows[i].investmentAmount;
      investment.customerID = rows[i].customerID;
      investment.ticketID = rows[i].ticketID;
      investments.push(investment);
      investment = { investmentID: "", investment_projectID: "", investmentAmount: "", customerID: "", ticketID: "" };
    }
  }
  res.status(200).json(investments);
});

router.get('/tickets', async (req, res) => {
  console.log("Body:", req.body);
  console.log("Query:", req.query);


/* rowCell.innerHTML = data[i].;
rowCell = row.insertCell(1);
rowCell.innerHTML = data[i].;
rowCell = row.insertCell(2);
rowCell.innerHTML = data[i].;
rowCell = row.insertCell(3);
rowCell.innerHTML = data[i].;
rowCell = row.insertCell(4);
rowCell.innerHTML = data[i].; */


let investment_projectID =  req.query.investment_projectID
  let customerID = req.query.customerid;

  // Initialize the sheet - doc ID is the long id in the sheets URL
  const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');
  // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
  await doc.useServiceAccountAuth({
    // env var values are copied from service account credentials generated by google
    // see "Authentication" section in docs for more info
    client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
    private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
  });

  await doc.loadInfo(); // loads document properties and worksheets
  console.log(doc.title);
  // await doc.updateProperties({ title: 'renamed doc' });

  const sheet = doc.sheetsByIndex[2]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
  console.log(sheet.title);
  console.log(sheet.rowCount);
  // read rows
  const rows = await sheet.getRows({ limit: 500, offset: 0 }); // can pass in { limit, offset }
  //let bFound = false;
  let tickets = [];
  let ticket = { ticketID: "", investment_projectID: "", customerID: "", ticketAmount: "", available: ""  };
  let rowCount = rows.length;
  for (let i = 0; i < rowCount; i++) {
    console.log(rows[i].investment_projectID);
    if (rows[i].investment_projectID == investment_projectID) {
      ticket.ticketID = rows[i].ticketID;
      ticket.investment_projectID = rows[i].investment_projectID;
      ticket.customerID = rows[i].customerID;
      ticket.ticketAmount = rows[i].ticketAmount;
      ticket.available = rows[i].available;
      tickets.push(ticket);
      ticket = { ticketID: "", investment_projectID: "", customerID: "", ticketAmount: "", available: ""  };
    }
  }
  res.status(200).json(tickets);
});

router.post("/orderInvestment", async (req, res) => {
  try {
    // Get user input
    const { ticketID, customerid, investment_projectID} = req.body;
    console.log (req.body);

    // Validate user input
    if (!(ticketID && customerid && investment_projectID)) {
      res.status(400).json({error:1, message: "Se deben capturar todos los campos" });
    }
    // Initialize the sheet - doc ID is the long id in the sheets URL
    const doc = new GoogleSpreadsheet('1sdgAH9zQo9OYhzw4VjQWAvm3o5-OXX5G2Et1tm4E0j0');

    // Initialize Auth - see https://theoephraim.github.io/node-google-spreadsheet/#/getting-started/authentication
    await doc.useServiceAccountAuth({
      // env var values are copied from service account credentials generated by google
      // see "Authentication" section in docs for more info
      client_email: process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || 'holding@holding-346100.iam.gserviceaccount.com',
      private_key: '-----BEGIN PRIVATE KEY-----\\nrsDIKXms/5K+rMMPVSGX4TeF\n-----END PRIVATE KEY-----\n',
    });

    await doc.loadInfo(); // loads document properties and worksheets
    const sheet = doc.sheetsByIndex[2]; // or use doc.sheetsById[id] or doc.sheetsByTitle[title]
    // read rows
    const rows = await sheet.getRows({ limit: 300, offset: 0 }); // can pass in { limit, offset }
    let bFound = false;
    let ticketAmount = 0;
    let rowCount = rows.length;
    for (let i = 0; i < rowCount; i++) {
      if (rows[i].ticketID == ticketID) {
          bFound = true;
          rows[i].available = 0;
          rows[i].customerID = customerid;
          ticketAmount = rows[i].ticketAmount;
          await rows[i].save(); // save updates
          break;
      }
    }
    if (bFound) {
      try{
        const sheet3 = doc.sheetsByIndex[3]; 
        let rows3 = await sheet3.getRows({ limit: 300, offset: 0 });
        rowCount = rows3.length;
        let investment = { investmentID: rowCount + 1 , investment_projectID: investment_projectID, investmentAmount: ticketAmount, customerID: customerid, ticketID: ticketID };
        console.log(investment);
        // append rows
        await sheet3.addRow(investment);
        res.status(201).json({error:0, message: "Se realizó la inversión" });
      }
      catch (err){
        res.status(409).json({error:1, message: err });
      }
    }
    else{
      res.status(409).json({error:1, message: "no se encontró el ID del ticket solicitado", });
      //res.redirect("https://mholding.herokuapp.com/error/?message=Ocurrió un error, no se encontró el ID del ticket solicitado");
    }
  } catch (err) {
    console.log(err);
  }
});

module.exports = router;
